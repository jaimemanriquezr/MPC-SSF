function probePClosure(season)
% H-candidate (G-probe finding): nutrient-CLOSED death rows — dissolved N/P
% release at death equals the biomass content implied by the growth rows
% (HET: N 0.0248, P 0.0141; PHO: N 0.06, P 0.01); POM carries no N/P,
% consistent with the P-free hydrolysis chain. Everything else identical to
% probePInventory. Quantifies the flip and the closure defect after the fix.
...
rx(iDH).StoichiometricCoefficients = dictionary( ...
    ["HET", "POM", "NH4", "HPO4"], [-1.0, 0.9123, 0.0248, 0.0141]);
rx(iDP).StoichiometricCoefficients = dictionary( ...
    ["PHO", "POM", "NH4", "HPO4"], [-1.0, 0.6316, 0.06, 0.01]);
...
for tEnd = [10, 30]
    ...
    B.release_deathPHO = 0.01*kDP*I(phoM, tEnd);
    B.release_deathHET = 0.0141*kDH*I(hetM, tEnd);
    ...
    res.(tag) = B;
    fprintf("PCLO %s %s: HPO4=%.3e biomP=%.4f totP=%.4f dInv=%.4f | in=%.4f outD=%.3e outP=%.3e | rel=%.4f (dPHO %.4f dHET %.4f end %.4f) | tau_P=%.2f d | grossPHO=%.4f | defect=%.4f\n", ...
...
save(fullfile(S, sprintf("pclo_%s.mat", season)), "res", "ts", "phoM", "hetM", "pomM", "hpo4M", "-v7");
fprintf("PCLO PROBE %s: flag=%s PHO(30d)=%.4f\n", season, r.Flag, phoM(end));
end
