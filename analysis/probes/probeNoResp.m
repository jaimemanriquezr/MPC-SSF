function probeNoResp(season, lightScale)
% = probeLight but with the phototroph endogenous respiration REMOVED entirely
% (death 0.09 O2-neutral remains the only biological PHO loss): isolates whether
% the winter/summer inversion needs the respiration term at all.
...
res.budget = B;
save(fullfile(S, sprintf("norsp_%s.mat", tag)), "res", "z", "ts", "o2", "nh4", "phiB", "phoM", "phoOut", "-v7");
disp(res);
fprintf("NORESP PROBE %s: flag=%s O2out=%.2f PHO=%.4f HET=%.4f\n", ...
    tag, r.Flag, res.o2_out_mean, res.pho_mass_end, res.het_mass_end);
fprintf("NORESP-BUDGET %s [kg/m2 over %g d]: supply=%.4f export=%.4f death=%.4f endog=%.4f dM=%.4f growth(residual)=%.4f (%.0f%% of supply)\n", ...
