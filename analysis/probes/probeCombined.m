function probeCombined(season, withHet)
...
save(fullfile(S, sprintf("combined_%s.mat", tag)), "res", "z", "ts", "o2", "nh4", "phiB", "-v7");
disp(res);
fprintf("COMBINED PROBE %s: flag=%s t=%.1f O2out=%.2f (%.2f-%.2f) bedmin=%.2f NH4=%.4f phib=%.3f sup=%.3f PHO=%.3f HET=%.3f\n", ...
