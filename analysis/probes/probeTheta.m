function probeTheta(season, thetaG)
...
rx(iP).TemperatureCorrectionFactor = thetaG;
...
rx = [rx; phoEndog];
tag = sprintf("%s_th%g", season, thetaG);
if withHet
...
res.budget = B;
save(fullfile(S, sprintf("theta_%s.mat", tag)), "res", "z", "ts", "o2", "nh4", "phiB", "phoM", "phoOut", "-v7");
disp(res);
fprintf("THETA PROBE %s: flag=%s O2out=%.2f PHO=%.4f HET=%.4f\n", ...
    tag, r.Flag, res.o2_out_mean, res.pho_mass_end, res.het_mass_end);
fprintf("THETA-BUDGET %s [kg/m2 over %g d]: supply=%.4f export=%.4f death=%.4f endog=%.4f dM=%.4f growth(residual)=%.4f (%.0f%% of supply)\n", ...
